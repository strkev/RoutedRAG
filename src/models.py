...
def get_connections_data() -> Dict[str, Any]:
    if os.path.exists(CONNECTIONS_FILE):
        try:
                        ...
                        "connections": [
                            {
                                ...
                                "default_model": raw.get("default_model", os.getenv("DEFAULT_MODEL", ""))
                            }
                        ]
                    }
            ...
            logger.error(f"Fehler beim Lesen von {CONNECTIONS_FILE}: {e}")

    env_base = os.getenv("BASE_URL", "").strip().strip('"').strip("'")
    if env_base:
        default_conn = {
            "id": "env_default",
            "name": "Umgebungsvariable (ENV)",
            "base_url": env_base,
            "api_key": os.getenv("API_KEY", ""),
            "default_model": os.getenv("DEFAULT_MODEL", "")
        }
        return {
            "default_connection": "env_default",
...
def save_connection_config(api_key: str, base_url: str, default_model: Optional[str] = None) -> bool:
    data = get_connections_data()
...
def evaluate_rules(user_msg: str, user_role: str | None = None) -> Tuple[str, str]:
    ...
    fallback_model = def_conn_cfg.get("default_model") or os.getenv("DEFAULT_MODEL", "")
    fallback_conn = def_conn_cfg.get("id", "")
...
def get_llm(model_name: Optional[str] = None, connection_id: Optional[str] = None, temperature: float = 0.3) -> ChatOpenAI:
    ...
    if not base_url:
        raise SetupRequiredException(
            "Keine aktive LLM-Verbindung eingerichtet. Bitte konfigurieren Sie unter 'Verbindungen' "
            "einen Provider oder setzen Sie BASE_URL in .env."
        )

    final_model = model_name or conn.get("default_model") or os.getenv("DEFAULT_MODEL", "")
    if not final_model:
        raise SetupRequiredException(
            "Kein Modell angegeben. Bitte konfigurieren Sie ein Standardmodell in den Verbindungseinstellungen oder via DEFAULT_MODEL in .env."
        )
...
def get_embeddings(model_name: Optional[str] = None) -> OpenAIEmbeddings:
    ...
    if not base_url:
        raise SetupRequiredException(
            "Keine LLM-Verbindung fuer Embeddings konfiguriert. Bitte richten Sie zuerst eine Verbindung ein."
        )

    final_model = model_name or conn.get("embedding_model") or os.getenv("EMBEDDING_MODEL", "")
    if not final_model:
        raise SetupRequiredException(
            "Kein Embedding-Modell konfiguriert. Bitte konfigurieren Sie EMBEDDING_MODEL in .env oder in den Einstellungen."
        )
...
class DynamicModelSelectionMiddleware(AgentMiddleware):
    def _apply_selection(self, request: ModelRequest) -> ModelRequest:
        ...
        if not dynamic_enabled:
            ...
            selected_model = selected_model or def_conn.get("default_model") or os.getenv("DEFAULT_MODEL", "")
            selected_conn = selected_conn or def_conn.get("id", "")
            logger.info(f"Manuelles Modell: {selected_model} (Provider: {selected_conn})")
        else:
            ...
            logger.info(f"Dynamisches Routing: {selected_model} (Provider: {selected_conn})")
    ...
    def wrap_model_call(self, request: ModelRequest, handler) -> ModelResponse:
        request = self._apply_selection(request)
...
dynamic_model_selection = DynamicModelSelectionMiddleware()
